...
const GlobalVerify = () => {
  ...
  const findDetails = () => {
    ...
    certificationContract.methods.getCertificateDetailsFromID(certificateID).call({
      ...
      // console.log(JSON.parse(details[0]));
      let tokenDetails = JSON.parse(details[0])
      if (tokenDetails.documentType !== "Template") {
        console.log(tokenDetails);
        setCertificateDetails(tokenDetails);
        setLoading(false);
      } else {
        setLoading(false);
        setCertificateDetails({title:"Error",type:"Template certificates can only be access through application.", properties:{ name:"",image:"",description:""}})
      }
    })
  }
  return (
