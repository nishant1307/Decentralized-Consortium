...
import DocumentInfo from "views/DocConekt/DocumentInfo";
            ...
            docContract.methods._tokensOfOwner(props.auth.user.publicKey).call({
                ...
                res.forEach(tokenId => {
                    ...
                    docContract.methods.getDocumentDetails(tokenId).call({
                        ...
                        let privateKey = await sessionStorage.getItem('privateKey');
                        let decryptedpassword = await decryptMessage(productDetails[0].encryptedPassword, privateKey)
                        let data = await decryptMessage(productDetails[0].encryptedData, decryptedpassword)
                        // console.log(JSON.parse(data), "data");
                        if (JSON.parse(data).DocType === props.location.state.documentType) {
                                            ...
                                            {loader ?
                                                ...
                                                <MaterialTable
                                                    ...
                                                    actions={[
                                                        ...
                                                        // },
                                                        {
                                                            tooltip: 'Add Selected Documents To Project',
                                                            icon: 'link',
...
export default connect(mapStateToProps, { openDocModal })(withStyles(dashboardStyle)(Shipping));
