...
function KYCComponent(props) {
  ...
  const submitForm = async () => {
    ...
    var transaction = {
      "to": registryAddress,
      "data": registryContract.methods.setOrganizationAdmin(uuidv1(), state.companyName, orgHash[0].hash, userHash[0].hash, JSON.stringify({ email: state.email, eula: eulaHash[0].hash })).encodeABI()
    };
